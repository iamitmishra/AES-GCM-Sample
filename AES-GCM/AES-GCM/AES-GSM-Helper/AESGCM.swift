...
public class AESGCM {
    ...
    init(key: Data, nonce: Data, tagSize: Int) throws {
        if tagSize != AESGCM.tagSize128 && tagSize != AESGCM.tagSize120 && tagSize != AESGCM.tagSize112 && tagSize != AESGCM.tagSize104 && tagSize != AESGCM.tagSize96 && tagSize != AESGCM.tagSize64 && tagSize != AESGCM.tagSize32 {
            throw AESGCMError.invalidTagSize
        }
        if key.count != AESGCM.keySize128 && key.count != AESGCM.keySize192 && key.count != AESGCM.keySize256 {
        ...
        self.h = UInt128(b: 0)
        self.h = try UInt128(raw: AESGCM.encryptBlock(key: key, data: AESGCM.emptyBlock))
        
        if nonce.count != AESGCM.standardNonceSize {
            self.counter = GaloisField.hash(h: h, a: Data(), c: nonce)
        } else {
            self.counter = AESGCM.makeCounter(nonce: nonce)
        }
    ...
    public func encrypt(auth: Data?, plaintext: Data) throws -> Data {
        if used { throw AESGCMError.instanceAlreadyUsed }
        
        let dataPadded: Data = GaloisField.padToBlockSize(plaintext)
        let blockCount: Int = dataPadded.count / AESGCM.blockSize
        let h: Data = try AESGCM.encryptBlock(key: key, data: AESGCM.emptyBlock)
        let eky0: Data = try AESGCM.encryptBlock(key: key, data: counter.getData())
        let authData: Data = (auth != nil ? auth! : Data())
        var ct: Data = Data()
        
        for i in 0..<blockCount {
            counter = counter.increment()
            let ekyi: Data = try AESGCM.encryptBlock(key: key, data: counter.getData())
            
            let ptBlock: Data = dataPadded[dataPadded.startIndex + i * AESGCM.blockSize..<dataPadded.startIndex + i * AESGCM.blockSize + AESGCM.blockSize]
            ct.append(AESGCM.xorData(d1: ptBlock, d2: ekyi))
        }
    ...
    public func decrypt(auth: Data?, ciphertext: Data) throws -> Data {
        ...
        let ct: Data = ciphertext[ciphertext.startIndex..<ciphertext.startIndex + ciphertext.count - AESGCM.blockSize]
        let givenT: Data = ciphertext[(ciphertext.startIndex + ciphertext.count - AESGCM.blockSize)...]
        ...
        if !AESGCM.tsCompare(d1: computedT, d2: givenT) {
            throw AESGCMError.authTagValidation
        }
        ...
        for i in 0..<blockCount {
            ...
            let ctBlock: Data = dataPadded[dataPadded.startIndex + i * AESGCM.blockSize..<dataPadded.startIndex + i * AESGCM.blockSize + AESGCM.blockSize]
            pt.append(AESGCM.xorData(d1: ctBlock, d2: ekyi))
        }
    ...
    private static func encryptBlock(key: Data, data: Data) throws -> Data {
        if data.count != AESGCM.blockSize {
            throw AESGCMError.invalidDataSize
        }
        ...
        let status = ct.withUnsafeMutableBytes { [ countCT = ct.count ]
        ctRaw in
            dataMutable.withUnsafeMutableBytes { dataRaw in
                keyMutable.withUnsafeMutableBytes{ keyRaw in
                    CCCrypt(operation, algorithm, options, keyRaw, key.count, nil, dataRaw, data.count, ctRaw, countCT, &num)
                }
            }
        }
        
        if status != kCCSuccess {
            throw AESGCMError.commonCryptoError(err: status)
        }
    ...
    private static func makeCounter(nonce: Data) -> UInt128 {
        ...
        result.append(nonce)
        result.append(AESGCM.initialCounterSuffix)
